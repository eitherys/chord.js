...
function log2(x) { return logb(x, 2); }

//Returns the decibel power of x.
function db(x) { return 10*logb(x, 10); }

//Powers of 2.
function lastPow2(x) { return Math.floor(log2(x)); }
function nextPow2(x){ return Math.ceil(log2(x)); }
function closestPow2(x) { return Math.floor(log2(x) + .5); }
...
function getFreq(n, o) { return af * Math.pow(2, (semitones[n]-semitones.A)/12) * Math.pow(2, o); }

//Returns the next semitone frequency of the given note, assuming it is tuned properly.
function nextNoteF(f) {	return f * Math.pow(2, 1/12); }

//Assumes that f is verified to be representative of n. Undefined results otherwise.
function getOctave(f, n) { return closestPow2(f/getFreq(n, 0)); }

//Returns whether the given frequency f is an octave of a specified note n within tolerance t.
